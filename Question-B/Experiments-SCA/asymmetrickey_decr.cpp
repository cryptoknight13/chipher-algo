...
void decrypt(PrivateRSAKey privKey, std::string &cipherText)
{
  char tmp[4];
...
int main(int argc, char *argv[])
{
  ...
  mpz_set_str(privKey.d, dStr.c_str(), 10);
  mpz_set_str(privKey.n, nStr.c_str(), 10); 
  ...
  cout << "------------------------ Time Report --------------------------------" << endl
       << endl;
  cout << "Total time for RSA Decryption: " << total.count() << " microSec." << endl;
  cout << "-----------------------------------------------------------------------" << endl
       << endl;
  return 0;
}
